...
func (scq *ServiceConfigQuery) QueryKeySets() *KeySetQuery {
	query := (&KeySetClient{config: scq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := scq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, serviceconfig.KeySetsTable, serviceconfig.KeySetsColumn),
		)
		fromU = sqlgraph.SetNeighbors(scq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (scq *ServiceConfigQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (scq *ServiceConfigQuery) FirstIDX(ctx context.Context) string {
...
func (scq *ServiceConfigQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (scq *ServiceConfigQuery) OnlyIDX(ctx context.Context) string {
...
func (scq *ServiceConfigQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (scq *ServiceConfigQuery) IDsX(ctx context.Context) []string {
	ids, err := scq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (scq *ServiceConfigQuery) Clone() *ServiceConfigQuery {
	...
	return &ServiceConfigQuery{
		...
		withKeySets: scq.withKeySets.Clone(),
...
func (scq *ServiceConfigQuery) WithKeySets(opts ...func(*KeySetQuery)) *ServiceConfigQuery {
	query := (&KeySetClient{config: scq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	scq.withKeySets = query
	return scq
}
...
func (scq *ServiceConfigQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*ServiceConfig, error) {
	...
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, serviceconfig.ForeignKeys...)
	}
	...
	if query := scq.withKeySets; query != nil {
		if err := scq.loadKeySets(ctx, query, nodes,
			func(n *ServiceConfig) { n.Edges.KeySets = []*KeySet{} },
			func(n *ServiceConfig, e *KeySet) { n.Edges.KeySets = append(n.Edges.KeySets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (scq *ServiceConfigQuery) loadService(ctx context.Context, query *ServiceQuery, nodes []*ServiceConfig, init func(*ServiceConfig), assign func(*ServiceConfig, *Service)) error {
	...
	nodeids := make(map[string][]*ServiceConfig)
	for i := range nodes {
		if nodes[i].service_service_config == nil {
			continue
		}
		fk := *nodes[i].service_service_config
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "service_service_config" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (scq *ServiceConfigQuery) loadKeySets(ctx context.Context, query *KeySetQuery, nodes []*ServiceConfig, init func(*ServiceConfig), assign func(*ServiceConfig, *KeySet)) error {
	...
	query.Where(predicate.KeySet(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(serviceconfig.KeySetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.service_config_key_sets
		if fk == nil {
			return fmt.Errorf(`foreign-key "service_config_key_sets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "service_config_key_sets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (scq *ServiceConfigQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(serviceconfig.Table, serviceconfig.Columns, sqlgraph.NewFieldSpec(serviceconfig.FieldID, field.TypeString))
	_spec.From = scq.sql
