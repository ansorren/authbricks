...
func (skq *SigningKeyQuery) QueryKeySet() *KeySetQuery {
	query := (&KeySetClient{config: skq.config}).Query()
...
func (skq *SigningKeyQuery) Clone() *SigningKeyQuery {
	...
	return &SigningKeyQuery{
		...
		withKeySet: skq.withKeySet.Clone(),
...
func (skq *SigningKeyQuery) WithKeySet(opts ...func(*KeySetQuery)) *SigningKeyQuery {
	query := (&KeySetClient{config: skq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	skq.withKeySet = query
	return skq
}
...
func (skq *SigningKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*SigningKey, error) {
	...
	if skq.withKeySet != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, signingkey.ForeignKeys...)
	}
	...
	if query := skq.withKeySet; query != nil {
		if err := skq.loadKeySet(ctx, query, nodes, nil,
			func(n *SigningKey, e *KeySet) { n.Edges.KeySet = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (skq *SigningKeyQuery) loadKeySet(ctx context.Context, query *KeySetQuery, nodes []*SigningKey, init func(*SigningKey), assign func(*SigningKey, *KeySet)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*SigningKey)
	for i := range nodes {
		if nodes[i].key_set_signing_keys == nil {
			continue
		}
		fk := *nodes[i].key_set_signing_keys
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "key_set_signing_keys" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
