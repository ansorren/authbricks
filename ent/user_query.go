...
// the "standard_claims" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithStandardClaims(opts ...func(*StandardClaimsQuery)) *UserQuery {
	query := (&StandardClaimsClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) loadStandardClaims(ctx context.Context, query *StandardClaimsQuery, nodes []*User, init func(*User), assign func(*User, *StandardClaims)) error {
	...
	query.withFKs = true
	query.Where(predicate.StandardClaims(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.StandardClaimsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_standard_claims
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_standard_claims" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
