...
type SessionQuery struct {
	...
	withAuthorizationPayload *AuthorizationPayloadQuery
...
// QueryAuthorizationPayload chains the current query on the "authorization_payload" edge.
func (sq *SessionQuery) QueryAuthorizationPayload() *AuthorizationPayloadQuery {
	query := (&AuthorizationPayloadClient{config: sq.config}).Query()
...
func (sq *SessionQuery) Clone() *SessionQuery {
	...
	return &SessionQuery{
		...
		withAuthorizationPayload: sq.withAuthorizationPayload.Clone(),
...
// WithAuthorizationPayload tells the query-builder to eager-load the nodes that are connected to
// the "authorization_payload" edge. The optional arguments are used to configure the query builder of the edge.
func (sq *SessionQuery) WithAuthorizationPayload(opts ...func(*AuthorizationPayloadQuery)) *SessionQuery {
	query := (&AuthorizationPayloadClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withAuthorizationPayload = query
	return sq
}
...
func (sq *SessionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Session, error) {
	var (
		...
		_spec       = sq.querySpec()
		loadedTypes = [1]bool{
			sq.withAuthorizationPayload != nil,
		}
	)
	...
	if query := sq.withAuthorizationPayload; query != nil {
		if err := sq.loadAuthorizationPayload(ctx, query, nodes, nil,
			func(n *Session, e *AuthorizationPayload) { n.Edges.AuthorizationPayload = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SessionQuery) loadAuthorizationPayload(ctx context.Context, query *AuthorizationPayloadQuery, nodes []*Session, init func(*Session), assign func(*Session, *AuthorizationPayload)) error {
	...
	query.withFKs = true
	query.Where(predicate.AuthorizationPayload(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(session.AuthorizationPayloadColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.session_authorization_payload
		if fk == nil {
			return fmt.Errorf(`foreign-key "session_authorization_payload" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "session_authorization_payload" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
